...
md`# Part 2) Sector List Examples
...
cell`function ringsToPolygons (circulationDS) {
  return circulationDS.polygons[0].inner_rings.map(
    ring => ring.map(({x,y}) => [x,y])
  )
}`
...
cell`function trapsToTrianglesAndColors (traps, scale = colorScale) {
  const triangles = [];
  const colors = [];
  for (let t of traps) {
    //if (t.w == 0) continue;
...
cell`function sectorListAndPolyToSvg (slist,...polygons) {
  let psvg = polyToSvg(...polygons);
  for (let ssvg of sectorListToSvg(slist)) psvg.append(ssvg);
  return psvg;
}`
...
cell`// Generates an svg for displaying a collection of polygons
function polyToSvg (...polygons) {
  const margin = 50;
  const [xmin,ymin,width,height] = polygons.length ? boundingBox(polygons.flat()):[0,0,1,1];
  const viewbox = ""+[xmin-margin,ymin-margin, width+margin*2,height+margin*2];
...
function _93(cell){return(
cell`function* sectorListToSvg (slist) {
  let prev = {x:0, y:0}, dup = 0;
  const rbase = 30, rincr = 10;
  for (let s of slist.sectors) {
    if (s.apex.x == prev.x && s.apex.y == s.apex.y) dup++;
...
cell`fieldRender = function (sectorList, options = {}) {
  // Default options
  let {
    showEdges = true,
    edgeColor = "black",
...
cell`inverseColorScale = function (colorScale) {
  return function (color) {
    let { r, g, b } = d3.color(color);
    let closest = 0;
    let closestDist = Number.MAX_VALUE;
...
cell`cssColorToWebGLColor = function (cssColor) {
  let { r, g, b, opacity } = d3.color(cssColor);
  return [r / 255, g / 255, b / 255, opacity];
}`
)}
...
cell`webGLColorToCssColor = function ([r, g, b, a]) {
  return \`rgba(\${r * 255},\${g * 255},\${b * 255},\${+a})\`;
}`
)}
...
cell`tooltipWebglCanvas = function (canvas, colorToValue) {
  let pixel = {
    width: 1,
    height: 1,
    data: new Uint8Array(4)
...
cell`createRegl = require('regl@1.4.2/dist/regl.js')`
)}
...
cell`glmatrix = import('https://unpkg.com/gl-matrix@3.3.0/esm/index.js?module')`
)}
...
cell`import {toolTipper} from "@esperanc/tippy-tooltips"`
)}
...
export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["cell"], _93);
